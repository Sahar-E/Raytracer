...
void saveImgAsJpg(const std::string &filename,
    ...
    for (int i = 0; i < n; ++i) {
        dataCopy[i*channelCount] = static_cast<unsigned char>(data[i].x0() * 255);
        dataCopy[i*channelCount + 1] = static_cast<unsigned char>(data[i].x1() * 255);
        dataCopy[i*channelCount + 2] = static_cast<unsigned char>(data[i].x2() * 255);
    }
